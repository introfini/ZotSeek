...
- Text after the last full stop in a long passage is no longer discarded. Splitting a passage into sentences kept only the parts that ended in `.`, `!` or `?`, so whatever trailed the final one was dropped. In ordinary prose that costs a few characters, measured at 0.15% of an affected passage across a sample of real PDFs. Where the punctuation all sits near the beginning, it costs everything: ninety thousand characters preceded by a single full stop were indexed as eleven. A leading full stop is no longer swallowed either. Documents affected by this need re-indexing to pick up the missing text.
...
- "Add Note Text to Existing Items", in the ZotSeek submenu of the item context menu, adds note text to items that are already in the index. Update Library Index skips anything already indexed, so this is the only way to apply note indexing to a library that was indexed before the setting was switched on. It follows the Index scope setting, reuses the existing embeddings so only note text is sent to the model, and can be paused, cancelled and resumed after an interrupted run like any other bulk indexing.

### Fixed
- Keyword search now finds text inside PDFs and notes, not just item metadata. Until now the keyword half of hybrid search asked Zotero for matches and then threw away every match that came from an attachment or a note, because it wanted papers back rather than files and notes. Those are exactly the items Zotero returns when the matching words are in a PDF's body or in a note, so the match was discarded instead of being credited to the paper it belonged to, and the search quietly behaved as if it only knew titles, creators, years and tags. Searching a sentence you remember from a paper, or a phrase you wrote in your own note, returned nothing. Each match is now attributed to the paper it belongs to, and a paper matched by several of its attachments appears once rather than once per file. Semantic search was never affected, and no re-indexing is needed.

- A keyword match now shows the passage that actually contains what you typed, instead of the passage that happens to be closest in meaning. A keyword hit arrives from Zotero without an excerpt, and ZotSeek picked one by comparing the paper's passages against the meaning of the query, which for a literal string such as a conference acronym is close to guessing. Searching "RCIS 2025" showed a Methods passage and claimed the match was on page 6 of the PDF, while the string exists only in a note on that item: a location the words are not on, stated with confidence. Passages containing the words you searched for are now preferred, and how closely a passage matches the meaning only decides between passages that contain the same words. Where nothing in the paper contains the words, the old behaviour is unchanged.

- Clicking the toolbar button while the search window is already open now brings that window to the front. It used to be left behind the main Zotero window, so the click appeared to do nothing at all.

- Note text written as a bulleted or numbered list keeps its short lines. Long notes are split into several entries before indexing, and that split drops very short paragraphs; turning every list item into its own paragraph meant a note full of brief bullets could lose most of them. List items now stay together as one paragraph, as table rows already do.

### Changed
- The ZotSeek commands in the item context menu are now grouped under a single **ZotSeek** submenu instead of sitting loose among Zotero's own entries, where they had grown to seven lines. **Find Similar Documents** stays at the top level, because it acts on the item you right-clicked and is the one most often wanted. Several labels lost the word ZotSeek now that the submenu carries it, and the indexing commands were made to read consistently: Open ZotSeek is now **Search Library**, Index Selected for ZotSeek is **Index Selected Items**, Update Library Index is **Update Entire Library**, and Remove from ZotSeek Index is **Remove from Index**.
- The Min similarity setting no longer filters keyword matches. It now applies only to the semantic half of hybrid search, which is where a similarity score means something. Version 1.21.0 had extended it to the whole result set, on the reasoning that a keyword hit with no score was slipping past a filter everything else was held to. That turned out to judge keyword hits by the wrong evidence: a keyword hit exists because the words are literally in the item, while the score measures how close the item's overall meaning is to what you typed, and for a conference acronym, a grant number or a dataset name those are barely related. On a 15,000-item library with the setting at 70%, searching "RCIS 2025" returned nothing at all, even though an article's note contains exactly that string. Keyword matches are now always returned, and they still arrive with the score and the excerpt that 1.21.0 gave them, shown for information rather than used to exclude them. Ordinary searches for a topic or an idea are unchanged.
