...
zotseek-pref-noteDelayLabel = Delay before re-indexing an edited note:
zotseek-pref-noteDelayDesc = Wait this long after the last change to a note before re-indexing its parent item.
...
zotseek-pref-minSimilarityDesc = % — Filters weak semantic matches (0-100). Applies to meaning-based matches only; a keyword match is never filtered by it.
...
zotseek-search-historyClear = Clear search history
zotseek-search-historyCleared = Search history cleared
...
zotseek-indexing-extractionFailed = ⚠ { $count } item(s) could not be read: { $items }
...
zotseek-indexing-completeFailed = ⚠ Could not be read: { $count } item(s). See the debug log for the error on each one.
...
zotseek-indexing-alreadyUpToDate = ✓ Already up to date
