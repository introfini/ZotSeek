...
zotseek-pref-indexNotes =
    .label = 同时索引子笔记
zotseek-pref-indexNotesDesc = 除上方模式外，还会索引条目所附笔记的文本。仅对此后索引的条目生效；如需为已索引的条目加入笔记，请使用条目右键菜单 ZotSeek 子菜单中的“为已索引项目添加笔记文本”。关闭后不再索引新的笔记文本，但已在索引中的笔记文本不会被移除。
...
zotseek-pref-noteDelayLabel = 重新索引已编辑笔记前的延迟：
zotseek-pref-noteDelayDesc = 笔记最后一次修改后等待这么长时间，再重新索引其所属条目。
...
zotseek-pref-minSimilarityDesc = % — 过滤语义相似度过低的匹配（0-100）。仅作用于语义匹配；关键词匹配永远不会被它过滤。
...
zotseek-search-historyClear = 清除搜索历史
zotseek-search-historyCleared = 搜索历史已清除
...
zotseek-indexing-extractionFailed = ⚠ { $count } 个条目无法读取：{ $items }
...
zotseek-indexing-completeFailed = ⚠ 无法读取：{ $count } 个条目。请查看调试日志以了解每个条目的错误。
...
zotseek-indexing-alreadyUpToDate = ✓ 已是最新
