...
zotseek-pref-indexNotes =
    .label = 同时索引子笔记
zotseek-pref-indexNotesDesc = 除上方模式外，还会索引条目所附笔记的文本。仅对此后索引的条目生效；如需为已索引的条目加入笔记，请使用条目右键菜单中的“将笔记文本加入索引”。关闭后不再索引新的笔记文本，但已在索引中的笔记文本不会被移除。
...
zotseek-search-history =
    .label = 🕘
    .tooltiptext = 最近的搜索
zotseek-search-historyEmpty = 没有最近的搜索
