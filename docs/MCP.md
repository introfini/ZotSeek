...
- `matchedChunk` is `null` when no excerpt or page is available; `page` and `textSource` may be absent within it. `textSource` is `"note"` when the match came from a note attached to the item rather than from its abstract or its PDF, and those chunks never carry a `page`, because a note has no page in the document. They carry `noteKey` instead — the matched note's own item key, which each note being chunked separately makes unambiguous. Select the note with `zotero://select/<prefix>/items/<noteKey>`, using the same `library`/`groups/<groupID>` prefix as the result's own `links.select`; the note may have been deleted since indexing, in which case that link lands on nothing and `links.select` is the one to offer. In `hybrid` mode this is now rare: hits found only by the keyword engine are matched against the item's own chunks before the response is built, so they carry an excerpt like any other result. It stays `null` for items that match on Zotero metadata but have never been indexed by ZotSeek.
...
These links work only on the machine where this Zotero instance is running. Used together with `matchedChunk.page`, they give an agent page-precise grounding: it can quote the matched passage and hand the user a link that opens the PDF right at that page. A match whose `textSource` is `"note"` has no `page`, so there is nothing to open the PDF at; quote the excerpt and link to the item instead.
